...
''',
    ...
    run_cmd(
        "git commit -m 'Initial commit: utils, analyzer, main'",
        cwd=repo_path,
        description="Commit initial files",
    )
...
'''
    ...
    run_cmd(
        "git commit -m 'Add caching functions to utils'",
        cwd=repo_path,
        description="Commit: Add caching functions",
    )
...
'''
    ...
    run_cmd(
        "git add analyzer.py", cwd=repo_path, description="Stage analyzer.py changes"
    )
    run_cmd(
        "git commit -m 'Add caching support to analyzer'",
        cwd=repo_path,
        description="Commit: Add caching support",
    )
...
if __name__ == \"__main__\":''',
    ...
    run_cmd(
        "git commit -m 'Add benchmarking to main'",
        cwd=repo_path,
        description="Commit: Add benchmarking",
    )
...
def test_yaver_workflow(repo_path, project_id):
    ...
    run_cmd(
        f"yaver project history {project_id} --limit 5",
        description="Show updated history",
    )
...
def test_after_modification(repo_path, project_id, mod_name):
    ...
    run_cmd(
        f"yaver project history {project_id} --limit 10", description="Show history"
    )
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            print(
                f"  4. ✓ Incremental analysis after second modification (analyzer.py)"
            )
