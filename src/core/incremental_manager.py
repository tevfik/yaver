...
class IncrementalAnalysisManager:
    ...
    def should_skip_analysis(self, project_id: str) -> tuple[bool, str]:
        ...
        # Check history
        should_reanalyze, reason = self.history_mgr.should_reanalyze(
            project_id, current_commit
        )
    ...
    def get_affected_functions(
        self, changed_files: List[Path], project_id: str, neo4j_adapter
    ) -> Set[str]:
        """
        ...
        try:
            with neo4j_adapter.driver.session() as session:
                for file_path in changed_files:
                    ...
                    """,
                        {"file_path": str(file_path), "project_id": project_id},
                    )
