...
def test_embeddings_auth():
    ...
    config = OllamaConfig()
    print(
        f"Config loaded. URL: {config.base_url}, User: {config.username}, Model: {config.model_embedding}"
    )

    if not config.username:
        print(
            "WARNING: No username configured. This test might pass even if auth is broken, as it falls back to no-auth."
        )
