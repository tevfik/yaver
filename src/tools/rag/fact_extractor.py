...
class FactTriple(BaseModel):
    ...
    predicate: str = Field(
        description="The relationship or property (e.g., IS_A, HAS_PROPERTY, CALLED_BY)"
    )
    object: str = Field(
        description="The entity or value that is the object of the fact"
    )
...
"""
    ...
    def _validate_and_filter_triples(
        ...
        for triple_data in raw_data["triples"]:
            ...
            if not subject or not predicate or not obj:
                logger.debug(
                    f"Skipping incomplete triple: subject={subject}, predicate={predicate}, object={obj}"
                )
                continue
    ...
    def extract_facts(self, text: str) -> List[FactTriple]:
        ...
        try:
            ...
            logger.info(
                f"Extracted {len(filtered_triples)} facts from text (validated {len(valid_triples)} triples)"
            )
