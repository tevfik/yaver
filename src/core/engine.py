...
class Engine:
    ...
    def execute_task(self, task: str, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            task_record = {"task": task, "status": "success", "result": str(result)}
            self.task_history.append(task_record)

            return {"status": "success", "output": result, "task": task}
...
class AgentEngine:
    ...
    def run(
        self, task: str, max_retries: int = 3, on_event: Optional[Callable] = None
    ) -> Dict[str, Any]:
